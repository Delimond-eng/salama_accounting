...
new Vue({
    ...
    methods: {
        ...
        async load() {
            ...
            try {
                ...
                const { data } = await get(`/reports/retards/daily/data?${params.toString()}`);

                this.range = { from: data?.from ?? "", to: data?.to ?? "" };
                const page = data?.retards ?? null;
                this.rows = page?.data ?? [];
    ...
    computed: {
        exportPdfUrl() {
            ...
            return `/reports/retards/daily/export/pdf?${params.toString()}`;
        },
        ...
        exportExcelUrl() {
            ...
            if (this.filters.to) params.set("to", this.filters.to);
            if (this.filters.station_id) params.set("station_id", this.filters.station_id);
            return `/reports/retards/daily/export/excel?${params.toString()}`;
        },
    },
});
